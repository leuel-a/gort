...
func VisualizeNumberArray(screen *ebiten.Image, numbers []int, current int, next int) {
	for i, number := range numbers {
		...
		x := float32((i + offset) * (rectangleWidth + spacerWidth))
		...
		if current == i {
			...
			if i+1 < len(numbers) {
				...
				z := float32((i + 1 + offset) * (rectangleWidth + spacerWidth))
...
func VisualizeNumberArraySorted(screen *ebiten.Image, numbers []int) {
	for i, number := range numbers {
		...
		vector.FillRect(screen, x, y, float32(rectangleWidth), height, color.RGBA{0xff, 0, 0, 0xff}, true)
		vector.StrokeRect(screen, x, y, float32(rectangleWidth), height, activeIndexStrokeWidth, color.RGBA{0, 0xff, 0, 0xff}, true)
